...
##################################### Pruning setting #################################################
parser.add_argument('--pruning_times', default=22, type=int, help='overall times of pruning')
...
def main():
    ...
    for state in range(start_state, args.pruning_times):
        ...
        # weight rewinding
        model.load_state_dict(initalization) # rewind, initialization is a full model architecture without masks
        prune_model_custom(model, current_mask)
        optimizer = torch.optim.SGD(model.parameters(), args.lr,
                                    momentum=args.momentum,
                                    weight_decay=args.weight_decay)
