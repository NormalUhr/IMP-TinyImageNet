    ...
    for name,m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            prune.ln_structured(
                ...
                name='weight',
                amount=px,
                dim=0, # 1: filter 0: channel
                n=1, # l1 loss
            )

def pruning_model_structured_l2_channel_wise(model, px):

    print('Apply structured L2 Pruning Globally (all conv layers) channel wise')
    # parameters_to_prune =[]
    for name,m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            prune.ln_structured(
                ...
                name='weight',
                amount=px,
                dim=0, # 0 refers to the channel wise
                n=2,
            )

def pruning_model_structured_l2(model, px):
